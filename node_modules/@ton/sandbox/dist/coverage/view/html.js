...
function formatGasCosts(gasCosts) {
...
function generateLineHtml(line, index, maxGasPerLine, totalGas) {
...
function generateInstructionRowsHtml(summary) {
...
function generateHtmlReport(coverage) {
    ...
    const maxGas = Math.max(...lines.map(line => line.info.$ === "Covered" ? line.info.gasCosts.reduce((sum, gas) => sum + gas, 0) : 0));
