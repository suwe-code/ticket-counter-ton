...
exports.mergeCoverages = mergeCoverages;
exports.mergeTwoLines = mergeTwoLines;
const core_1 = require("@ton/core");
function buildLineInfo(trace, asm) {
...
function mergeCoverages(...coverages) {
    if (coverages.length === 0) {
        return {
            code: new core_1.Cell(),
            lines: [],
        };
    }
    let allLines = coverages[0]?.lines ?? [];
    for (const coverage of coverages.slice(1)) {
        allLines = mergeTwoLines(allLines, coverage.lines);
    }
    return {
        code: coverages[0]?.code ?? new core_1.Cell(),
        lines: allLines,
    };
}
;
function mergeTwoLines(first, second) {
